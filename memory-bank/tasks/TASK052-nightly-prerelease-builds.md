...
- Every nightly release includes a plain-language download guide and labels each artifact with its platform, architecture, and whether to install, extract, or run it directly.
- Release notes identify the exact linked commit, its title, and its body when the commit includes a description.
