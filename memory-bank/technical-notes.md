...
- A watched folder's starting baseline is reported with its eligible-file count. It remains non-uploading by default, but the operator can explicitly choose **Process existing files**; that action reuses the same active-channel, current-inventory, duplicate-review, managed-copy, resumable-queue, and source-cleanup boundaries as newly discovered files.
