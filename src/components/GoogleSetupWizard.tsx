  ...
  return (
    <section aria-describedby="google-setup-detail" aria-labelledby="google-setup-heading" aria-modal="true" className="fixed top-1/2 left-1/2 z-20 grid w-[calc(100%-2rem)] max-w-[48rem] -translate-x-1/2 -translate-y-1/2 gap-3.5 rounded-xl border border-[#bfcfe7] bg-white p-5 shadow-[0_20px_52px_rgba(27,50,83,0.22)] max-compact:max-h-[calc(100vh-1.25rem)] max-compact:overflow-y-auto max-compact:p-4" role="dialog">
      ...
      <section className="grid gap-2.5 rounded-lg border border-[#dce5f1] bg-[#f7f9fd] px-4 py-3.5 [&>h3]:m-0 [&>h3]:text-[1rem] [&>h3]:tracking-[-0.025em] [&>h3]:text-[#1e2d48] [&>p]:mt-0 [&>p]:text-[0.79rem] [&>p]:leading-[1.5] [&>p]:text-[#617086]" aria-live="polite">
        ...
        <h3>{current.title}</h3>
        <p>{current.detail}</p>
        {step === 0 && <button className="justify-self-start cursor-pointer rounded-md border border-brand bg-brand px-3 py-2 text-[0.77rem] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 max-compact:w-full" disabled={!isTauri || busy} onClick={() => void openSetupPage("account")} type="button">Open Google account</button>}
        {step > 0 && step < 5 && <button className="justify-self-start cursor-pointer rounded-md border border-brand bg-brand px-3 py-2 text-[0.77rem] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 max-compact:w-full" disabled={!isTauri || busy} onClick={() => void openSetupPage("cloud")} type="button">Open Google Cloud Console</button>}
        ...
        {importing && <button className="justify-self-start cursor-pointer rounded-md border border-brand bg-brand px-3 py-2 text-[0.77rem] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 max-compact:w-full" disabled={busy} onClick={onOpenConnectedAccount} type="button">Open Connected account</button>}
      ...
      <footer className="flex flex-wrap items-center justify-end gap-2 max-compact:flex-col-reverse max-compact:items-stretch max-compact:[&_button]:w-full">
        <button className="cursor-pointer rounded-md border border-[#cdd4df] bg-white px-3 py-2 text-[0.77rem] font-semibold text-[#34405a] disabled:cursor-not-allowed disabled:opacity-50" disabled={step === 0 || busy} onClick={() => setStep((currentStep) => currentStep - 1)} type="button">Back</button>
        {!importing && <button className="cursor-pointer rounded-md border border-brand bg-brand px-3 py-2 text-[0.77rem] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50" disabled={busy} onClick={() => setStep((currentStep) => currentStep + 1)} type="button">Continue</button>}
        <button className="mr-auto cursor-pointer rounded-md border border-[#cdd4df] bg-white px-3 py-2 text-[0.77rem] font-semibold text-[#34405a] disabled:cursor-not-allowed disabled:opacity-50 max-compact:mr-0" disabled={busy} onClick={onDismiss} type="button">Continue later</button>
      </footer>
    </section>
  );
}
