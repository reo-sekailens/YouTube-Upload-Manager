...
export function TransferPanel({ onConnectionChange, onNotice }: TransferPanelProps) {
  ...
  return <section className="panel transfer-panel" aria-labelledby="transfer-heading">
    ...
    <div className="transfer-panel__grid">
      <section><h3>Duplicate metadata archive</h3><p>Exports SHA-256 hashes, item titles, and synchronized YouTube inventory with gzip compression. It excludes media, local paths, refresh tokens, OAuth client secrets, and resumable sessions.</p><div><button className="secondary-action" disabled={busy} onClick={() => void exportArchive()} type="button">Export compact archive</button><button className="secondary-action" disabled={busy} onClick={() => void importArchive()} type="button">Import archive</button></div></section>
