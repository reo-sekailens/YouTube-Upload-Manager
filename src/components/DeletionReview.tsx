...
export function DeletionReview({ activeChannel, busy = false, onNotice }: DeletionReviewProps) {
  ...
  const selectableVideos = videos.filter((video) => !requestedVideoIds.has(video.videoId));
  const allSelectableSelected = selectableVideos.length > 0 && selectableVideos.every((video) => selectedVideoIds.has(video.videoId));
  ...
  const toggleSelectAll = () => {
    setSelectedVideoIds(allSelectableSelected ? new Set() : new Set(selectableVideos.map((video) => video.videoId)));
  };
  const reviewSelected = () => {
    const selectedVideos = videos.filter((video) => selectedVideoIds.has(video.videoId) && !requestedVideoIds.has(video.videoId));
